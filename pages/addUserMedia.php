...
if (isset($_FILES['Filedata'])) {
    ...
    if (in_array($fileExtension, $imageFileTypes)) {
        ...
        if ($imageIsSaved) {
            ...
            echo json_encode(array('success' => 'Added successfully', 'url' => constant('URL_UPLOADS_MEDIA') . $mediaFileName . '?token=' . $token, 'progressId' => $progressId, 'type' => 'image', 'token' => $mediaToken));
        } else {
        ...
        if (is_dir($targetFolder) && is_writable($targetFolder)) {
            ...
            if (move_uploaded_file($tempFile, $targetMediaFile)) {
                ...
                echo json_encode(array('success' => 'Added successfully', 'url' => constant('URL_UPLOADS_MEDIA') . 'processing-video.png' . '?token=' . $token, 'progressId' => $progressId, 'type' => 'video', 'token' => $mediaToken));
